...
export const cart = {
	...
	addCartItem(form: any, bundle = false) {
		...
		// gift card recipient
		let recipientCheckbox = document.querySelector(
			`#recipient-checkbox`
		) as HTMLInputElement;
		let recipientObj;
		if (recipientCheckbox && recipientCheckbox.checked) {
			...
			let recipientMessage = document.querySelector(
				`#recipient-message`
			) as HTMLInputElement;
			...
			if (!recipientName.value || !recipientEmail.value) {
				(this.error_title = "Error"),
					(this.error_message =
						"Please fill out name and email of gift card recepient"),
					(this.show_alert = true);
				this.cart_loading = false;
				return;
			}
	...
	addWithId(
		...
		let properties = document.getElementsByClassName(
			`custom-input_${productHandle}`
		...
		const serialize = function (obj: StringIndexable<ICartItem>): string {
			...
			for (const p in obj) {
				if (obj.hasOwnProperty(p)) {
					str.push(
						`${encodeURIComponent(p)}:${encodeURIComponent(obj[p].toString())}`
					);
				}
			}
			return str.join(",") + "&";
